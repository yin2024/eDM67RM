...
public class BookInsertServlet extends HttpServlet {
	...
	try {
	    ...
	    BookBean bb = new BookBean(0, title, author, price, 1.0, cID, fileName, bookNo);
    ...
    /*
     * public String getFileName(final Part part) { for (String content :
     ...
     * } return null; } public void exploreParts(Collection<Part> parts,
     * HttpServletRequest req){ try { for (Part part: parts){ String name =
     * part.getName(); String contentType = part.getContentType(); String value
     * = ""; long size = part.getSize(); // 上傳資料的大小，即上傳資料的位元組數 InputStream is
     * =part.getInputStream(); if (contentType != null) { // 表示該part為檔案 //
     * 取出上傳檔案的檔名 String filename = getFileName(part); //
     * 將上傳的檔案寫入到location屬性所指定的資料夾 part.write(filename); } else { //
     * 表示該part為一般的欄位 // 將上傳的欄位資料寫入到location屬性所指定的資料夾，檔名為"part_"+ name
     * part.write("part_"+ name); value = req.getParameter(name); }
